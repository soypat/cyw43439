...
func main() {
	...
	logger.Info("DHCP complete", slog.String("addr", string(ipv4.AppendFormatAddr(nil, dhcpResults.AssignedAddr4))))
	...
	err = stack.RegisterUDP4(&mdnsclient, [4]byte{}, mdns.Port)
	if err != nil {
	...
	mdaddr, err := cywnet.DoMDNS(&mdnsclient, remoteHostname+".local", timeout, retries)
	if err != nil {
		panic(err)
	}
	println("Address discovered for", remoteHostname, mdaddr.String())
